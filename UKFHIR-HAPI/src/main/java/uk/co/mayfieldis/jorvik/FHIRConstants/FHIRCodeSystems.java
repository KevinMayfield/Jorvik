package uk.co.mayfieldis.jorvik.FHIRConstants;

public class FHIRCodeSystems {
			    
	    public final static String URI_NHS_NUMBER_ENGLAND = "urn:fhir.nhs.uk:id/NHSNumber";
	    
	    public final static String URI_NHS_NUMBER_SCOTLAND = "http://fhir.nhs.scot.uk/Patient/NHSNumber";
	    
	    public final static String URI_CHI_NUMBER_SCOTLAND = "http://fhir.nhs.scot.uk/Patient/CHINumber";
	    
	    public final static String URI_SNOMED = "http://snomed.info/sct";
	    
	    public final static String URI_OID_NHS_PERSONNEL_IDENTIFIERS = "urn:oid:2.16.840.1.113883.2.1.4.4";
	    
	    public final static String URI_OID_NHS_SPECIALTIES = "urn:oid:2.16.840.1.113883.2.1.6.8";
	    
	    public final static String URI_NHS_SPECIALTIES = "http://england.nhs.uk/HL7v2/zu015";
	    
	    public final static String URI_NHS_GMP_CODE = "urn:fhir.nhs.uk/id/GeneralPractitionerPPDCode";
	    
	    public final static String URI_NHS_OCS_ORGANISATION_CODE = "urn:fhir.nhs.uk/id/ODSOrganisationCode";
	
	    public final static String URI_NHS_ORGANISATION_TYPE = "http://fhir.datadictionary.nhs.uk/data_dictionary/organisation_type";
	    
	    public final static String URI_NHS_ACTIVE_PERIOD = "http://fhir.nhs.uk/period/active";
}
